...
- [ ] **Enforce separation of duties — risk-tiered** (Admin §A1, §8). Reviewer ≠ signer (block same user_id) — but tier by risk so it's not blanket-expensive: **High** (prescribing/safety guidance, regulated SOPs) = full two-person independent sign-off; **Medium** (templates, non-safety sections) = single reviewer + sign-off; **Low** (typo/formatting) = light-touch/auto-approve + post-hoc audit. Each item carries a risk tier that drives its path.
- [ ] **Signer qualification must match content type** (Admin §A2). **Clinical** content (guidelines, clinical note templates) → final sign-off only by a **clinically-qualified lead** (registered clinician); **governance/admin** content (SOPs, policies) → admin/governance lead OK. System checks signer role/registration vs content type — don't let a non-clinical admin clinically sign clinical guidance.
...
- [ ] **Case image upload + AI PII check** (§10, §5.4). Case submission needs **1–5 clinical photos** (essential for dermatology) with **AI pre-check flagging identifiable images** (faces/tattoos/jewellery/background) before submit, **auto-strip EXIF**, consent checkbox, encrypted UK/EU storage, access-limited + audited + deletable. AI must catch identifiers in **images**, not just text.
- [ ] **Two-tier tenancy model** (Admin §A7, §5.7). Separate **clinic-scoped** users (clinicians, clinic admins — see only their clinic via RLS `clinic_id`) from **platform-scoped** users (MDT panel + Clinickly super-admin — serve all clinics, sit above tenancy). Don't lump the panel/central admin into a single clinic. Confirm isolation with ≥2 clinics; enforce area-locking via RLS, not UI hiding.
- [ ] **Panel member = full credentialing record** (Admin §A3, §4). "Add member" currently captures only name/role/routing specialty. Add: **professional registration/PIN (verify against register), CV upload, photo, signed contract + terms, DBS, indemnity, verified qualification tags** — one profile; this is the §4B vetting gate.
