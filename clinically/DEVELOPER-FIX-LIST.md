...
- [ ] **Upgrade CPD log/export to GPhC-native record types** (§5.8 decision 9 Jul, from the real GPhC forms). Records not minutes (6/yr = 4 CPD [≥2 planned] + 1 peer discussion + 1 reflective account). Typed entries matching the forms' exact questions; auto-map module→planned, case learning→unplanned, **MDT response→peer discussion (panel member = named peer, standing consent)**; reflective account scaffolded vs the year's standards; **AI criteria pre-check** ("benefit to service users with an example" present?). Replace the minutes-led framing ("45 CPD minutes") with records-led.
