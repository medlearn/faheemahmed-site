...
- [ ] **SOP "Build with AI" flow** (§7). **MOSTLY THERE (R2.9):** "Build for my clinic" entry point on SOP cards (correctly absent on note templates); outputs are per-clinic, clinic-lead-signed, gap-checked vs GPhC/CQC/MHRA/NHS, versioned. Remainder: walk through the builder (questions → AI draft → edit) to verify the middle; **add duplicate guard** (re-build → warn/v2, never a second v1); **clinic name from clinic profile, not free text** ("RIverside" typo).
